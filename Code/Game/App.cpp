	...
	, m_consoleCaretBlinkTimer(0.f)
{
	...
	g_theConfig->Initialize("UI.config");
...
// 	Rgba clearColor(134, 180, 205, 255);
	Rgba clearColor = Rgba::GREY;
	...
	g_theRenderer->SetProjectionMatrix(g_theRenderer->CreatePerspectiveProjectionMatrix(0.f, 1920.f, 0.f, 1080.f, 0.01f, 500.f, 70.f));
	g_theRenderer->SetViewMatrix(m_game->m_theCamera.GetViewMatrix());
	g_theRenderer->SetDirectionalLight(Vector3(0.3f, -1.f, 0.f), Rgba::WHITE, 0.5f);
// 	g_theRenderer->SetDirectionalLight(Vector3(0.3f, -1.f, 0.f), Rgba(179, 164, 90), 1.5f);
	g_theRenderer->SetAmbientLight(Rgba(255, 255, 255), 0.3f);
	g_theRenderer->SetSpecularConstants(16.f, 1.f);
	g_theRenderer->SetShader(m_shaders[0]);

	g_theRenderer->SetSampler(m_theSampler);

	g_theRenderer->EnableBlend(BLEND_SRC_COLOR, BLEND_ZERO);
...
void App::ReloadShaders()
{
	...
	m_shaders.push_back(g_theRenderer->CreateShaderFromHLSLFile("Data/HLSL/lit.hlsl"));
...
void App::DrawConsoleLog() const
{
	...
	g_theRenderer->SetShader(m_shaders[1]);
	g_theRenderer->SetTexture(nullptr);
